...
module.exports = {
    ...
    deleteMovie: function(req, res) {
        Actor.findOneAndUpdate({ _id: req.params.aId }, req.body, function (err, actor) {
            if (err) return res.status(400).json(err);
            if (!actor) return res.status(404).json();

            let index = actor.movies.indexOf(req.params.mId);
    ...
    cascadeDeleteMovies: function (req, res) {
        Actor.findOneAndDelete({ _id: req.params.id }, function (err, actor) {
                if (err) return res.status(400).json(err);
                if (!actor) return res.status(404).json();

            let query = { _id: { $in: [ actor.movies ] } };
